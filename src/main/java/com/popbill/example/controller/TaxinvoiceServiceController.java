...
public class TaxinvoiceServiceController {
    ...
    public String registRequest(Model m) {
        ...
        // [역발행시 필수] 공급받는자 문서번호, 1~24자리 (숫자, 영문, '-', '_') 조합으로 사업자 별로 중복되지 않도록 구성
        taxinvoice.setInvoiceeMgtKey("20250711-BOOT003");
    ...
    public String getInfos(Model m) {
        ...
        try {
            TaxinvoiceInfo[] taxinvoiceInfos = taxinvoiceService.getInfos(CorpNum, mgtKeyType, MgtKeyList, UserID);
    ...
    public String getPrintURL(Model m) {
        ...
        // 세금계산서 문서번호
        String mgtKey = "202507118-BOOT001";
    ...
    public String attachFile(Model m) {
        ...
        try {
            Response response = taxinvoiceService.attachFile(CorpNum, mgtKeyType, mgtKey, displayName, stream, UserID);
            m.addAttribute("Response", response);
        } catch (PopbillException e) {
            m.addAttribute("Exception", e);
            return "exception";
